...
class Agent(object):
    ...
    def _init(self, sess, ob_space, ac_space, gaussian_fixed_var=True):
        ...
        with tf.variable_scope("vf"):
            # ob_scan = tf.clip_by_value((self.scan_ph - self.ob_scan_rms.mean) / self.ob_scan_rms.std, -5.0, 5.0)
            ...
            concat_net = tl.layers.InputLayer(
                tf.concat([ob_pose, ob_goal, ob_lac, cnn_output], axis=1), 
                name='concat')
            vf_net = tl.layers.DenseLayer(concat_net, 64, act=tf.nn.tanh, name='fc')
            vf_net = tl.layers.DenseLayer(vf_net, 1, name='final')
        ...
        with tf.variable_scope('policy'):
            ...
            concat_net = tl.layers.InputLayer(
                tf.concat([ob_pose, ob_goal, ob_lac, cnn_output], axis=1), 
                name='concat')
            ac_net = tl.layers.DenseLayer(concat_net, 64, act=tf.nn.tanh, name='fc')

            if gaussian_fixed_var and isinstance(ac_space, gym.spaces.Box):
                mean = tl.layers.DenseLayer(ac_net, pdtype.param_shape()[0]//2, 
                    name='final')
                ...
                ac_net = tl.layers.DenseLayer(
                    ac_net, pdtype.param_shape()[0], name='final')
