...
class RayCast
{
  ...
  std::shared_ptr<CircleSim> circle_sim_ = nullptr;
  double m_per_px_;
  cv::Point map_offset_;

  // Default configuration variables
  double ray_min_ = 1.0;    // m
  double ray_max_ = 200.0;  // m
...
public:
  ...
  RayCast(double ray_min, double ray_max, double angle_min, double angle_max, double angle_inc, double noise)
  {
    ray_min_ = ray_min;
    ray_max_ = ray_max;
  ...
  void SetCircles(light_scan_sim::CircleList &circles, light_scan_sim::MaterialList &materials)
  {
    circle_sim_ = std::make_shared<CircleSim>(circles, materials);
  }
