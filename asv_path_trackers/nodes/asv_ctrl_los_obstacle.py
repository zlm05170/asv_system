...
class LOSGuidanceROS(object):
    ...
    def __init__(self,
        ...
        self.controller.shipname = self.shipname
        # rospy.loginfo('Parameter %s has value %s', rospy.resolve_name('~ship_init_state'), shipname)
        # rospy.loginfo('Parameter %s has value %s', rospy.resolve_name(shipname+'initial_state'), self.init_state)
        # print("init state len: %d" %(len(self.init_state)))
        # print("init_state: %f, %f" %(self.init_state[0], self.init_state[1]))

        self.init_waypoints()

    def init_waypoints(self):
        self.controller.wp[0] = self.init_state[0]
        self.controller.wp[1] = self.init_state[1]
        self.controller.wp_new = self.controller.wp.copy()
        while np.hypot(self.controller.wp_new[1] - self.controller.wp[1],
                       self.controller.wp_new[0] - self.controller.wp[0]) < 100.:
            self.controller.wp_new = np.random.uniform(-150., 150., 2)
    ...
    def _visualize_waypoints(self, switched):
        ...
        mk.color.a = .3
        if int(self.shipname[-1]) == 1:
            mk.color.r = 1.
            mk.color.g = 0.6
            mk.color.b = 0.
        elif int(self.shipname[-1]) == 2:
            mk.color.r = 0.
            mk.color.g = 1.
            mk.color.b = 0.
        elif int(self.shipname[-1]) == 3:
            mk.color.r = 0.
            mk.color.g = 0.
            mk.color.b = 1.
    ...
    def _odom_callback(self, data):
        self.odom = data
        if self._first_odom:
            self.controller.init_start(self.odom.pose.pose.position.x,
...
class LOSGuidance(Controller):
    ...
    def __init__(self,
        ...
        self.wp = np.array([0., 0.])
        self.wp_new = np.array([0., 0.])
    ...
    def circle_of_acceptance(self, x, y):
        return \
            (x - self.wp[0])**2 + \
            (y - self.wp[1])**2 < self.R2

    def check_reach_wp_new(self, x, y):
        return \
            (x - self.wp_new[0])**2 + \
            (y - self.wp_new[1])**2 < self.R2

    def progress_along_path(self, x, y):
        return \
            np.abs((self.wp[0] - x)*np.cos(self.Xp) +
                   (self.wp[1] - y)*np.sin(self.Xp)) < self.R

    def update_waypoints(self):
        self.wp = self.wp_new.copy()
        dist = np.hypot(self.wp_new[1] - self.wp[1],
                        self.wp_new[0] - self.wp[0])
        while dist < 120. or dist > 200.:
            self.wp_new = np.random.uniform(-180., 180., 2)
            dist = np.hypot(self.wp_new[1] - self.wp[1],
                       self.wp_new[0] - self.wp[0])
    ...
    def update(self, x, y):
        ...
        if self.switching_criterion(x, y):
            while self.switching_criterion(x, y):
                if not self.check_reach_wp_new(x, y):
                    # There are still waypoints left
                    print("%s reached the waypoint %d: (%.2f, %.2f)!" % (
                        self.shipname, self.cWP, self.wp[0], self.wp[1]))

                    new_course = np.arctan2(self.wp_new[1] - self.wp[1],
                                            self.wp_new[0] - self.wp[0])
                    ...
                    if self.R2 < 50000:
                        print("Sth Wrong, you should not be in here!!!")
                        print("Last Waypoint i.e. Goal %d: (%.2f, %.2f) reached!" % (
                            self.cWP, self.wp[0], self.wp[1]))
                        self.R2 = np.Inf
                        return 0, self.Xp, False

        dist_to_start = np.hypot(x-self.start[0], y-self.start[1])
        # print("counter: %d, distance to start: %.2f" % (self.counter, dist_to_start))
        self.counter += 1
