...
class LOSGuidanceROS(object):
    ...
    def set_waypoints(self, wps):
        self.wp = wps
        self.controller.wp = np.copy(wps[0])
        # self.controller.goal = np.copy(wps[1])
...
class LOSGuidance(Controller):
    ...
    def __init__(self,
        ...
        self.wpt_sub = rospy.Subscriber(
            "wpt_target", Waypoint, self.WptCallback, queue_size=10)
        self.wpt_fb_pub = rospy.Publisher(
            "wpt_feedback", Bool, queue_size=10)
    ...
    def WptCallback(self, msg):
        ...
        self.wp[0] = msg.x
        self.wp[1] = msg.y
        print "recieve waypoint: %f, %f" % (self.wp[0], self.wp[1])
    ...
    def update(self, x, y):
        ...
        if self.switching_criterion(x, y):
            while self.switching_criterion(x, y):
                aa = (x - self.goal[0])**2 + (y - self.goal[1])**2
                print "check reach goal: {}, x-{}, y-{}, goal_x-{}, goal_y-{}".format(
                    aa, x, y, self.goal[0], self.goal[1])
                if not self.check_reach_goal(x, y):
                    # There are still waypoints left
                    print "Waypoint %d: (%.2f, %.2f) reached!" % (self.cWP,
                                                                  self.wp[0],
                                                                  self.wp[1])
                    last_x = copy.deepcopy(self.wp[0])
                    last_y = copy.deepcopy(self.wp[1])
                    fb = Bool()
                    fb.data = True
                    self.wpt_fb_pub.publish(fb)
                    rospy.sleep(3.)
                    # TODO: the next wpt - the curr wpt
                    new_course = np.arctan2(self.wp[1] - last_y,
                                            self.wp[0] - last_x)
